...
$sql = $con->prepare("SELECT * FROM `users` WHERE (`contact` = :userid ||  `email` = :userid) && `pass` = :enc_pass ");
...
function register($Fname,$Lname,$mobile,$email,$pass){
global $con, $date, $time;
$data = [];
$salt = 'salt0';
$password = $pass;
...
$sql = $con->prepare("INSERT INTO users SET f_name = :Fname, l_name = :Lname, contact = :mobile, email = :email, pass = :enc_pass, status = '1' ");
...
insert("INSERT INTO tbl_accounts SET pk = '".$_SESSION['id']."', pk_type = 'USER', amount = '".$common_settings['customer_signup_bonus']."', from_status = 'new user registration', to_status = 'signup bonus added' ");
...
function tbl_wishlist($product_id,$user_id,$consultant_id){
global $con;
// product id decode
$decode_pid = decode($product_id);
...
$check_cart_func = check($check_cart);
if ($check_cart_func['status']==true) {
$sql_update = "UPDATE tbl_wishlist SET product_qty = product_qty+1 WHERE user_id = '".$user_id."' && product_id = '$decode_pid'";
update($sql_update);
return '<span>Item added to wishlist</span>';
}else{ // else for cart,
$sql_ins_cart = "INSERT INTO tbl_wishlist SET user_id = '".$user_id."', product_id = '$decode_pid', cosultant_id_seller = '".$cosultant_id_seller."', consultant_id_supplier = '".$consultant_id_supplier."'  ";
insert($sql_ins_cart);
return '<span>Item added to wishlist</span>';
}
}else{ // else checking for product
...
$call_tbl_wishlist = tbl_wishlist($product_w,$_SESSION['id'],$consultant_w);
...
if (isset($_POST['pass_token']) && isset($_POST['old_pass'])) {
    ...
    // check user old pass
    $pass_conditions = ["id"=>$_SESSION['id'], 'pass' => $old_pass];
    ...
    if ($data['data']==true) {
      ...
      if ($new_pass == $conf_pass) {
        ...
        update("UPDATE users SET pass = '$new_pass' WHERE id = '".$_SESSION['id']."' ");
